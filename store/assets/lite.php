...
if (version_compare(PHP_VERSION, '5.4.0', '<'))
	die('Require PHP 5.4 or higher');
if (!extension_loaded('sqlite3') && !extension_loaded('pdo_sqlite'))
	die('Install sqlite3 or pdo_sqlite extension!');
session_name('Lite');
session_start();
$bg = 2;
$step = 20;
...
class DBT {
	...
	public function __construct($db) {
		$ty = self::$litetype[0];
		if (extension_loaded($ty))
			$this->ltype = $ty;
		else
			$this->ltype = self::$litetype[1];
		if ($this->ltype == $ty)
...
class ED {
	...
	public function fieldtype($slct = '') {
		...
		foreach ($fieldtype as $fdk => $fdtype) {
			if (is_array($fdtype)) {
				...
				foreach ($fdtype as $fdty)
					$ft .= "<option value='$fdty'" . (($slct != '' && $fdty == $slct) ? " selected" : "") . ">$fdty</option>";
				$ft .= "</optgroup>";
			}
		}
		return $ft;
	...
	public function redir($way = '', $msg = []) {
		if (count($msg) > 0) {
			foreach ($msg as $ks => $ms)
				$_SESSION[$ks] = $ms;
		}
		header('Location: ' . $this->path . $way);
		exit;
	}
	...
	public function check($level = [], $param = []) {
		if (!empty($_SESSION['ltoken'])) {
			if ($_SESSION['ltoken'] != base64_encode(md5($_SERVER['HTTP_USER_AGENT'] . $this->passwd)))
				$this->redir("50", ['err' => "Wrong password"]);
		...
		if (in_array(2, $level)) { //check table
			...
			if (!$ist)
				$this->redir("5/$db", ['err' => "Table not exist"]);
		}
		if (in_array(3, $level)) { //check field
			...
			if (!in_array($this->sg[3], $meta))
				$this->redir($param['redir'] . "/$db/$tb", $err);
			if (isset($this->sg[5])) {
				if (!in_array($this->sg[5], $meta))
					$this->redir($param['redir'] . "/$db/$tb", $err);
			}
		}
		if (in_array(4, $level)) { //check pagination
			if (!is_numeric($param['pg']) || $param['pg'] > $param['total'] || $param['pg'] < 1)
				$this->redir($param['redir'], ['err' => "Invalid page number"]);
		}
		if (in_array(5, $level)) { //check view,trigger
			$sp = ['view', 'trigger'];
			$sg3 = $this->sg[3];
			if ($op != 49 && ($op == 40 && $sg3 != $sp[0]) || ($op == 41 && $sg3 != $sp[1]))
				$this->redir("5/$db");
			$q_sp = $this->con->query("SELECT 1 FROM sqlite_master WHERE name='" . $this->sg[2] . "' AND type='$sg3'", true)->fetch();
			if (!$q_sp)
				$this->redir("5/$db", ['err' => "Not available object"]);
		}
	}
	public function menu($db = '', $tb = '', $left = '', $sp = []) {
		$str = "";
		if ($db == 1 || $db != '')
			$str .= "<div class='l2'><ul><li><a href='{$this->path}'>Databases</a></li>";
		if ($db != '' && $db != 1)
			$str .= "<li><a href='{$this->path}31/$db'>Export</a></li><li><a href='{$this->path}5/$db'>Tables</a></li>";
		$dv = "<li class='divider'>---</li>";
		if ($tb != "")
			$str .= $dv . "<li><a href='{$this->path}10/$db/$tb'>Structure</a></li><li><a href='{$this->path}20/$db/$tb'>Browse</a></li><li><a href='{$this->path}21/$db/$tb'>Insert</a></li><li><a href='{$this->path}24/$db/$tb'>Search</a></li><li><a class='del' href='{$this->path}25/$db/$tb'>Empty</a></li><li><a class='del' href='{$this->path}26/$db/$tb'>Drop</a></li>"; //table
		if (!empty($sp[1]) && $sp[0] == 'view')
			$str .= $dv . "<li><a href='{$this->path}40/$db/" . $sp[1] . "/view'>Structure</a></li><li><a href='{$this->path}20/$db/" . $sp[1] . "'>Browse</a></li><li><a class='del' href='{$this->path}49/$db/" . $sp[1] . "/view'>Drop</a></li>"; //view
		if ($db != '')
			$str .= "</ul></div>";

		if ($db != "" && $db != 1) {
			$str .= "<div class='l3 auto'><select onchange='location=this.value;'><optgroup label='databases'>";
			foreach ($this->listdb() as $udb)
				$str .= "<option value='{$this->path}5/$udb'" . ($udb == $db ? " selected" : "") . ">$udb</option>";
			$str .= "</optgroup></select>";
			$q_ts = [];
			$c_sp = !empty($sp) ? count($sp) : "";
			if ($tb != "" || $c_sp > 1) {
		...
		if ($left == 1)
			$str .= "<div class='col1'><h3>Run sql</h3>
	...
	public function pg_number($pg, $totalpg) {
		if ($totalpg > 1) {
			if ($this->sg[0] == 20)
				$link = $this->path . "20/" . $this->sg[1] . "/" . $this->sg[2];
			elseif ($this->sg[0] == 5)
				$link = $this->path . "5/" . $this->sg[1];
			$pgs = '';
			$k = 1;
			while ($k <= $totalpg) {
	...
	public function imp_csv($fname, $fbody) {
		...
		if (!$exist)
			$this->redir("5/" . $this->sg[1], ['err' => "Table not exist"]);
		$fname = $this->sanitize($fname);
		$e = [];
		if (@is_file($fbody))
			$fbody = file_get_contents($fbody);
		$fbody = $this->utf($fbody);
		$fbody = preg_replace('/^\xEF\xBB\xBF|^\xFE\xFF|^\xFF\xFE/', '', $fbody);
		//delimiter
		$delims = [';' => 0, ',' => 0];
		foreach ($delims as $dl => &$cnt)
			$cnt = count(str_getcsv($fbody, $dl));
		...
		foreach ($rows as $k => $rw) {
			if ($k > 0) {
				...
				foreach ($rw as $r)
					$e1 .= (is_numeric($r) ? $r : "'" . str_replace("'", "''", $r) . "'") . ',';
				$e[] = substr($e1, 0, -1) . ");";
			}
		}
		if (empty($e))
			$this->redir("5/" . $this->sg[1], ['err' => "Query failed"]);
		return $e;
	}
	public function imp_json($fname, $fbody) {
		...
		$jr = '';
		foreach ($lines[0] as $k => $li)
			$jr .= $k . ",";
		foreach ($lines as $line) {
			$jv = '';
			foreach ($line as $ky => $el)
				$jv .= (is_numeric($el) ? $el : "'" . $el . "'") . ",";
	...
	public function imp_xml($fname, $fbody) {
		...
		foreach ($data as $dt) {
			...
			if ($co != '' && $va != '')
				$e[] = "INSERT INTO '" . (string) $tt['name'] . "'(" . substr($co, 0, -1) . ") VALUES(" . substr($va, 0, -1) . ");";
		}
		return array_merge($sq, $e);
	}
	public function imp_sqlite($fname, $fbody) {
		if ($fbody != '') {
			if (substr(file_get_contents($fbody), 0, 15) != "SQLite format 3" && substr($fbody, 0, 15) != "SQLite format 3")
				$this->redir('', ['err' => "No SQLite file"]);
			$file = pathinfo($fname);
			...
			if (is_uploaded_file($fbody)) {
				...
				$sfile = fopen($new, "wb");
				if (!$sfile)
					$this->redir('', ['err' => "Unable to create sqlite file"]);
				fwrite($sfile, $fbody);
				fclose($sfile);
				$this->redir('', ['ok' => "SQLite file uploaded"]);
			}
...
switch ($ed->sg[0]) {
	...
	case "2": //create db
		...
		if ($ed->post('dbc', '!e')) {
			...
			if (@is_file($ed->dir . $db . $ed->ext))
				$ed->redir("", ['err' => "DB already exist"]);
			$ed->con = DBT::factory($ed->dir . $db . $ed->ext);
			if (@is_file($ed->dir . $db . $ed->ext))
				$ed->redir("", ['ok' => "Created DB"]);
			else
	...
	case "3": //rename db
		...
		if ($ed->post('rdb', '!e') && $ed->sanitize($ed->post('rdb')) != $db) {
			...
			$ed->redir("5/$db", ['err' => "DB name must not be empty"]);
		break;
	...
	case "6": //create table
		...
		if ($ed->post('ctab', '!e') && !is_numeric(substr($ed->post('ctab'), 0, 1)) && $ed->post('nrf', '!e') && $ed->post('nrf') > 0 && is_numeric($ed->post('nrf'))) {
			...
			if ($q_ch)
				$ed->redir("5/$db", ['err' => "Name already exists"]);
	...
	case "9":
		...
		if ($ed->post('copytab', '!e')) { //copy table
			...
			if ($q_cc)
				$ed->redir("10/$db/$tb", ['err' => "Table already exists"]);
		...
		if ($ed->post('rtab', '!e')) { //rename table
			...
			if ($ren_tb === false)
				$ed->redir("10/$db/$tb", array('err' => "Can't rename"));
		...
		if (!empty($ed->sg[3])) { //drop index
			...
			if ($q_ii === false)
				$ed->redir("10/$db/$tb", ['err' => "Can't drop index"]);
			else
				$ed->redir("10/$db/$tb", ['ok' => "Successfully dropped"]);
		}
	...
	case "11": //add field
		...
		if ($ed->post('add', 'i')) {
			...
			if (!empty($f1)) {
				...
				$ed->redir("11/$db/$tb", ['err' => "Empty field name"]);
			if ($e)
				$ed->redir("10/$db/$tb", ['ok' => "Successfully added"]);
			else
				$ed->redir("10/$db/$tb", ['err' => "Can't add this field"]);
		} else {
	...
	case "12": //change field
		...
		if ($ed->post('change', 'i')) {
			...
			foreach ($q_t as $e) {
				if ($e[1] == $fn1) {
					if (empty($fn2) || is_numeric(substr($fn2, 0, 1)))
						$ed->redir("10/$db/$tb", ['err' => "Not a valid field name"]);
			...
			foreach ($q_it as $r_it)
				$ed->con->exec("DROP " . $r_it[0] . " " . $r_it[1]);
		...
		if (isset($ed->sg[3]) && $ed->sg[3] >= 0) {
			foreach ($q_fk as $r_fk) {
				if ($r_fk['id'] == $ed->sg[3]) {
					...
					foreach ($fkty as $fkt)
						echo "<option value='$fkt'" . ($r_fk['on_delete'] == $fkt ? " selected" : "") . ">$fkt</option>";
					echo "</select></td><td><select name='urule'>";
					foreach ($fkty as $fkt)
						echo "<option value='$fkt'" . ($r_fk['on_update'] == $fkt ? " selected" : "") . ">$fkt</option>";
					echo "</select></td></tr>";
				}
			}
		} else {
			...
			foreach ($fkty as $fkt)
				echo "<option value='$fkt'>$fkt</option>";
			echo "</select></td><td><select name='urule'>";
			foreach ($fkty as $fkt)
				echo "<option value='$fkt'>$fkt</option>";
			echo "</select></td></tr>";
	...
	case "20": //table browse
		...
		if ($count->fetch() == false)
			$ed->redir("5/$db", ['err' => "No data"]);
		$all = $count->fetch();
		$totalpg = ceil($all / $step);
		if (empty($ed->sg[3])) {
			$pg = 1;
	...
	case "21": //insert row
		...
		if ($ed->post('save', 'i') || $ed->post('save2', 'i')) {
			...
			if ($ed->post('save2', 'i'))
				...
				$rr = 20;
			if ($q_inn === false)
				$ed->redir("$rr/$db/$tb", ['err' => "Can't insert"]);
			else
				$ed->redir("$rr/$db/$tb", ['ok' => "Successfully inserted"]);
		} else {
	...
	case "22": //edit row
		...
		$nu = $ed->sg[3];
		if (empty($nu))
			$ed->redir("20/$db/$tb", ['err' => "Can't edit empty field"]);
		$id = ($ed->sg[4] == "isnull" ? "" : base64_decode($ed->sg[4]));
		$nu1 = (empty($ed->sg[5]) ? "" : $ed->sg[5]);
		$id1 = (empty($ed->sg[6]) ? "" : base64_decode($ed->sg[6]));
		...
		if ($ed->post('edit', 'i')) {
			...
			$ed->con->exec("PRAGMA foreign_keys=OFF");
			if ($q_edd === false)
				$ed->redir("20/$db/$tb", ['err' => "Can't update"]);
			else
				$ed->redir("20/$db/$tb", ['ok' => "Successfully updated"]);
		} else {
	...
	case "23": //delete row
		...
		$ed->con->exec("PRAGMA foreign_keys=OFF");
		if ($exec_dr->last())
			$ed->redir("20/$db/$tb", ['ok' => "Deleted row"]);
		else
			$ed->redir("20/$db/$tb", ['err' => "Delete row failed"]);
		break;
	...
	case "24": //search
		...
		if ($ed->post('search', 'i')) { //post
			...
			foreach ($q_se as $r_se) {
				if ($ed->post($r_se['name'], '!e') || in_array($ed->post('cond__' . $r_se['name']), $cond4)) {
					...
					} elseif (in_array($cd, $cond3))
						$search_cond[] = $fd . " " . $cd . " ('" . $po . "')";
					elseif (in_array($cd, $cond4))
						$search_cond[] = $fd . " " . $cd;
					else
						$search_cond[] = $fd . " " . html_entity_decode($ed->post('cond__' . $fd)) . " '" . $po . "'";
				}
			}
		...
		$conds = "";
		foreach ($cond as $cnd)
			$conds .= "<option value='" . $cnd . "'>" . $cnd . "</option>";
		...
		if ($ed->post('qtxt', '!e')) { //in textarea
			...
			if (preg_match('/^\b(select)\b/is', $qtxt)) { //run select
				...
				if ($q_sel) {
					...
					foreach ($q_sel[0] as $k => $r_sel)
						echo "<th>$k</th>";
					echo "</tr>";
					foreach ($q_sel as $r_sel) {
						$bg = ($bg == 1) ? 2 : 1;
						echo "<tr class='r c$bg'>";
						foreach ($r_sel as $r_se)
							echo "<td>$r_se</td>";
						echo "</tr>";
					}
					echo "</table>";
				} else
					$ed->redir("5/$db", ['err' => "Wrong query"]);
			} else {
			...
			if (empty($_FILES['importfile']['tmp_name'])) {
				...
				if ($fext == 'sql') { //sql file
					...
					if (@function_exists('gzopen')) {
						...
						if ($e_ext == 'sql')
							$e = preg_split($rgex, $ed->utf($e), -1, PREG_SPLIT_NO_EMPTY);
						elseif ($e_ext == 'csv')
							$e = $ed->imp_csv($entr['filename'], $e);
						elseif ($e_ext == 'json')
							$e = $ed->imp_json($entr['filename'], $e);
						elseif ($e_ext == 'xml')
							$e = $ed->imp_xml($entr['filename'], $e);
						elseif (in_array($e_ext, ['db', substr($ed->ext, 1)]))
							$ed->imp_sqlite($file['filename'], $e);
						else
					...
					if (is_resource($zip)) {
						...
						if (zip_entry_open($zip, $zip_entry, "rb")) {
							...
							if ($file['filename'] == $zentry) {
								...
								if (!empty($zn[2]) && $zn[2] == 'sql')
									$e = preg_split($rgex, $ed->utf($buf), -1, PREG_SPLIT_NO_EMPTY);
								elseif (!empty($zn[2]) && $zn[2] == 'csv')
									$e = $ed->imp_csv($zn[1], $buf);
								elseif (!empty($zn[2]) && $zn[2] == 'json')
									$e = $ed->imp_json($zn[1], $buf);
								elseif (!empty($zn[2]) && $zn[2] == 'xml')
									$e = $ed->imp_xml($zn[1], $buf);
								else
									$ed->redir("5/$db", ['err' => "Disallowed extension"]);
			...
			foreach ($e as $qry) {
				...
				if (!empty($qry)) {
					...
					if (extension_loaded(DBT::$litetype[0])) {
						...
						if ($exc && ($e_exc[0] === '00000' || $e_exc[0] === '01000'))
							++$q;
						else
							$out .= $fa . $qry;
					}
	...
	case "32": //export
		...
		if ($ffmt[0] == 'sql') { //data sql
			...
			if (!empty($fopt)) {
				foreach ($tbs as $tb) {
					if (in_array('structure', $fopt))
						$sql .= $ed->tb_structure($tb, $fopt); //begin structure
					$val = '';
			...
			$sign = ($ffmt[0] == 'csv1' ? ',' : ';');
			if (empty($tbs[0]))
				$ed->redir("31/$db", ['err' => "Select a table/view"]);
			foreach ($tbs as $tb) {
				$sq = '';
				...
				if (count($q_jso) > 0) {
					...
					foreach ($q_jso as $k_jso => $r_jso) {
						...
						foreach ($r_jso as $k_jo => $r_jo)
							$jh .= '"' . $k_jo . '":' . (is_numeric($r_jo) ? $r_jo : '"' . preg_replace(["/\r\n|\r|\n/", "/\t/", "/'/", "/\"/"], ["\\n", "\\t", "''", "&quot;"], $r_jo) . '"') . ',';
						$sq .= substr($jh, 0, -1) . '},';
					}
					$sq = substr($sq, 0, -1) . ']';
				}
			...
			foreach ($tbs as $tb) {
				...
				if (in_array('structure', $fopt)) {
					...
					foreach ($q_doc[0] as $r_k => $r_doc)
						$wh .= '<th>' . $r_k . '</th>';
					$wh .= '</tr>';
					foreach ($q_doc as $r_doc) {
						$wh .= '<tr>';
						foreach ($r_doc as $r_d1)
							$wh .= '<td>' . $r_d1 . '</td>';
						$wh .= '</tr>';
					}
					$wh .= '</table><br>';
  ...
  $xh = '<Worksheet ss:Name="' . $tb . '">
    <Table>
      ...
      $xh .= '<Row>';
        ...
        $xh .= '<Cell><Data ss:Type="' . (is_numeric($r_x2) ? 'Number' : 'String') . '">' . htmlentities($r_x2) . '</Data></Cell>';
        $xh .= '</Row>';
      }
      $sql .= $xh . '
    </Table>
...
$checksum = 8 * 32;
for ($i = 0; $i
< strlen($ctxt); $i++) $checksum +=ord($ctxt[$i]); $ctxt .=sprintf("%06o", $checksum) . "\0 " ; $ctxt .=str_repeat(" \0", 512 - strlen($ctxt)); $ctxt .=$sqa; $ctxt .=str_repeat("\0", 511 - ($len + 511) % 512); fwrite($tmpf, $ctxt); fseek($tmpf, 0); $fs=fstat($tmpf); $sq .=fread($tmpf, $fs['size']); fclose($tmpf); } $fname=$db . ".tar" ; $sql=$sq . pack('a1024', '' ); } $sql=gzencode($sql, 9); header('Content-Encoding: gzip'); } elseif ($ftype=="zip" ) { //zip $zty="application/x-zip" ; $zext=".zip" ; $info=[];
  $ctrl_dir=[]; $eof="\x50\x4b\x05\x06\x00\x00\x00\x00" ; $old_offset=0; if(is_array($sql)) $sqlx=$sql; else $sqlx[$fname]=$sql; foreach ($sqlx as $qname=> $sqa) {
				$ti = getdate();
				if ($ti['year'] < 1980) {
					$ti['year'] = 1980;
					$ti['mon'] = 1;
	...
	case "33": //blob download
		...
		if (empty($ed->sg[7])) {
			...
			$ph = $ed->sg[7];
			$nu1 = " AND " . $ed->sg[5] . "='" . base64_decode($ed->sg[6]) . "'";
		}
		$q_ph = $ed->con->query("SELECT $ph FROM $tb WHERE $nu='$id'$nu1", true)->fetch();
		$r_ph = base64_decode($q_ph);
		$len = strlen($r_ph);
		if ($len >= 2 && $r_ph[0] == chr(0xff) && $r_ph[1] == chr(0xd8)) {
			$tp = 'image/jpeg';
			$xt = '.jpg';
		} elseif ($len >= 3 && substr($r_ph, 0, 3) == 'GIF') {
			$tp = 'image/gif';
			$xt = '.gif';
		} elseif ($len >= 4 && substr($r_ph, 0, 4) == "\x89PNG") {
			$tp = 'image/png';
			$xt = '.png';
		} else {
	...
	case "40": //view
		if (!isset($ed->sg[2]) && !isset($ed->sg[3])) { //add
			...
			if ($ed->post('uv1', '!e') && $ed->post('uv2', '!e')) {
				...
				if (is_numeric(substr($tb, 0, 1)))
					$ed->redir("40/$db", ['err' => "Not a valid name"]);
				$exi = $ed->con->query("SELECT 1 FROM sqlite_master WHERE name='$tb'", true)->fetch();
				if ($exi)
					$ed->redir("40/$db", ['err' => "This name exist"]);
				$vstat = $ed->post('uv2');
				$stat = $ed->con->exec($vstat);
				if ($stat === false)
					$ed->redir("40/$db", ['err' => "Wrong statement"]);
				$v_cre = $ed->con->exec("CREATE VIEW $tb AS $vstat");
				if ($v_cre === false)
					$ed->redir("40/$db", ['err' => "Can't create view"]);
				else
			...
			if ($ed->post('uv1', '!e') && $ed->post('uv2', '!e')) {
				$tb = $ed->sanitize($ed->post('uv1'));
				if (is_numeric(substr($tb, 0, 1)))
					$ed->redir("5/$db", ['err' => "Not a valid name"]);
				$exi = $ed->con->query("SELECT 1 FROM sqlite_master WHERE name='$tb'", true)->fetch();
				if ($exi && $tb != $r_uv[1])
					$ed->redir("5/$db", ['err' => "This name exist"]);
				$vstat = $ed->post('uv2');
				$stat = $ed->con->exec($vstat);
				if ($stat === false)
					$ed->redir("5/$db", ['err' => "Wrong statement"]);
	...
	case "41": //trigger
		...
		foreach ($q_tbs as $tgt) {
			$tg = $tgt[0];
			if (!in_array($tg, $deny))
				echo "<option value='$tg'" . (str_replace('"', '', trim($r_tge[4])) == $tg ? " selected" : "") . ">$tg</option>";
		}
		echo "</select></td></tr><tr><td>Time</td><td><select name='utg2'>";
		$tm = ['BEFORE', 'AFTER'];
		foreach ($tm as $tn)
			echo "<option value='$tn'" . ($r_tge[2] == $tn ? " selected" : "") . ">$tn</option>";
		echo "</select></td></tr><tr><td>Event</td><td><select name='utg3'>";
		$evm = ['INSERT', 'UPDATE', 'DELETE'];
		foreach ($evm as $evn)
			echo "<option value='$evn'" . ($r_tge[3] == $evn ? " selected" : "") . ">$evn</option>";
...
<div class="l1 ce"><a href="http://edmondsql.github.io">edmondsql</a></div>
